...
int main(int argc, char *argv[])
{
  ...
  /*We read the response from the server. If response is "WELCOME", then everything went right. */
  ...
  {
    printf("\n---- MENU ----\n");
    printf("  1. Upload video to server for video censoring\n");
    printf("  2. Upload video to server for audio censoring\n");
    printf("  3. Upload video to server for full censoring\n");
    ...
    write(sock, &op, sizeof(op));
      ...
      /*We prompt the user to input the desired video file and store it in the path variable. */
      ...
      memset(response, 0x00, 1024); // Reset the response buffer.
      ...
      /*If response is the string "SEND_FILE" then all is okay and we can proceed to upload the .mp4 */
      ...
      {
        /*Now we parse out the size of the censored video from the server response which is always in format "SUCCESS VIDEO_SIZE" */
        ...
        /*Meanwhile we ask the user to provide a file with same extension to save the downloaded file to local storage... */
        do
        {
      ...
      {
        /*If there is an error we report it to the user and continue the looping, so that user can try again. */
